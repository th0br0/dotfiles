    ...
    , handleEventHook = ewmhDesktopsEventHook <+> fullscreenEventHook <+> focusFollow <+>
                    (\e -> case e of
                        PropertyEvent{ ev_window = w } -> do
                            isURXVT <- runQuery (className =? "Terminator") w
...
myXPConfig :: XPConfig
myXPConfig = greenXPConfig { font = "xft:ProFont:size=13:autohint=true" }
...
myKeys c =
    ...
    ,("M-S-r", withFocused (sendMessage . tallWindowAlt) >> sendMessage Taller)
    ,("M-r", withFocused (sendMessage . wideWindowAlt) >> sendMessage Wider )
 
    ,("M-v", toggleFF)
 
    ,("M-S-b", restart "/home/th0br0/bin/obtoxmd" True)
    ,("M-S-d", restart "terminator -x xmonad" False)
...
mediaKeys = [("<XF86AudioPlay>", do mpcAct "toggle"; when amarok $ spawn "amarok -t"),
	     ...
	     ("<XF86MonBrightnessUp>", spawn "sudo lcd-brightness +"),
	     ("<XF86MonBrightnessDown>", spawn "sudo lcd-brightness -")
             ]
    where mpcAct c = do
...
instance ExtensionClass HostPrompt where
    initialValue = HostPrompt "th0n4s"
...
promptHost = mkXPrompt (HostPrompt "") myXPConfig (return . compl) (XS.put . HostPrompt)
    where compl s = nub $ filter (s `isPrefixOf`) ["127.0.0.1","th0n4s"]
--------------------------------------------------------------
...
myTopicConfig = TopicConfig
  ...
  , topicActions = M.fromList $
      [  ("music"  , spawn "mpc && terminator -x ncmpc")
       , ("irc"    , spawnOn "irc" "xchat")
       , ("web"    , spawnOn "web" "google-chrome")
       , ("pdf"    , spawnOn "pdf" "okular")
       , ("gimp"   , spawnHere "gimp")
       , ("skype"  , spawnHere "skype")
      -- , ("kolab"     , spawnOn "kolab" "kontact")
      ]
  }
